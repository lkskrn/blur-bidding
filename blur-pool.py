...
def blurAddToPool(privatekey):
    ...
    def add_to_pool():
        try:
            ...
            contract_txn = contract.functions.deposit().buildTransaction({
                'value': web3.toWei(poolAmount, 'ether'),
                'nonce': nonce,
            })
    ...
    def withdraw_from_pool():
        try:
            ...
            nonce = web3.eth.get_transaction_count(address_wallet)
            amount = web3.toWei(poolAmount, 'ether')
            gasLimit = web3.eth.estimate_gas({'to': Web3.toChecksumAddress(address_wallet), 'from': Web3.toChecksumAddress(
                address_wallet), 'value': web3.toWei(0.001, 'ether')}) + random.randint(50000, 100000)
            contract_txn = contract.functions.withdraw(amount).buildTransaction({
                'nonce': nonce,
                "gas": gasLimit,
    ...
    add_to_pool()  # Uncomment to add to pool
    # withdraw_from_pool()  # Uncomment to withdraw from pool
